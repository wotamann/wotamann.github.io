<template>
    <v-container class="container-box">  
     <h4>Button Group/Toggle Example </h4>
    
  
      <!-- FORM-BASE-COMPONENT -->
        <v-form-base 
          :model= "myModel"
          :schema= "mySchema"
          cols="12"
        />
      <!-- FORM-BASE-COMPONENT -->
      
      
      <!-- Stuff  -->    
      <infoline :model="myModel" :schema="mySchema"/>
    </v-container>
  </template>
  
  <script setup>
    import vFormBase from '@/vFormBase.vue'
    import { ref } from 'vue'
    import Infoline from '@/components/infoline'
    
    
    const optionsIcon = [
      { icon: 'mdi-wifi', value: 'wifi', size:'x-large', }, 
      { icon: 'mdi-printer', value: 'print', size:'x-large', },
      { icon: 'mdi-bell-outline', value: 'bell', size:'x-large' },
    ]
    const optionsText = [
      { value: 'blue', color:'blue', text:'blue',  }, 
      { value: 'green', color:'green', text:'green', },
      { value: 'red', color:'red', text:'red',},
    ]
    const optionsTextIcon = [
      { iconL: 'mdi-format-align-left', text:'Left',value: 'L' }, 
      { iconL:  'mdi-format-align-center', text:'Center', value: 'C' },
      { iconR: 'mdi-format-align-right', text:'Right', value: 'R' },
    ]
  
    const myModel = ref({
      btnToggleText: 'left',  // if no value defined - lowercase of text:'Left' will be used as value
      btnToggleMultiple:['print']
        //unset props will be auto-added       
    })
    
    // control Display Order by Order in Schema
    const mySchema = ref({
          btnToggleText: { el: 'btntoggle', options: optionsText, divided:true, mandatory:true  },
          btnToggleTextIcon: { el: 'btntoggle', options: optionsTextIcon },
          btnToggleMultiple: { el: 'btntoggle', options: optionsIcon, multiple:true, variant:'outlined', color:'info' },
    })
  
  </script>
  