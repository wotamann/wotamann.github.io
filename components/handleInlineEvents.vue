<template>
  <v-container class="container-box">  
   <h4>Use Inline-Events defined in Schema</h4>
  ...
  const mySchema = ref({
    ...
    demo_updateModelValue: {
      ...
      onUpdateModelValue: (e,obj,ec) => {
        console.log('LOG INF MODEL', e);
        console.log('LOG INF MODEL', obj.value);
        console.log('LOG INF MODEL', ec);
        
        return ec?.modelValue?.toUpperCase()
        return e.toUpperCase()
        // return
        // return obj?.value.toUpperCase() // works only on native event 'INPUT' not on 'UPDATE:MODELVALUE'
        // return 'NEWVALUE' 
