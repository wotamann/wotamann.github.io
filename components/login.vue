...
<script setup>
  ...
  // const formOptions = { type:'submit', class:'pa-10', style:'backgroundColor:#dddddd' }
  const formOptions = { class:'pa-10', style:'backgroundColor:#dddddd' }
  const visible = ref(false)
  const visibleIcon = computed(()=> visible.value ? 'mdi-eye-off' : 'mdi-eye')
  const visibleType = computed(()=> !visible.value ? 'password' : 'text' )
  const visibleTypeReadWrite = computed({
    get: ()=> !visible.value ? 'password' : 'text',
